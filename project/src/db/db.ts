...
class SuiviVenteDB extends Dexie {
  ...
  usernames!: Table<username, number>;
  ...
  constructor() {
    ...
    this.version(1).stores({
      ...
      usernames: '++id, usernamename, role',
      logs: '++id, date, username, action'
    });
  ...
  async populateInitialData() {
    ...
    await this.usernames.bulkAdd([
      { usernamename: 'admin', password: 'admin123', role: 'admin' },
      { usernamename: 'username', password: 'username123', role: 'username' }
    ]);
  ...
  async logAction(username: string, action: string, details: string) {
